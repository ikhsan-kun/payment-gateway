...
import handlePayment from "../Data/data";
...
const produkList = [
  ...
  },
  {
    ...
    harga: "Rp 2.000.000",
    gambar:
      "https://images.unsplash.com/photo-1517841905240-4729888e3b2e?auto=format&fit=crop&w=400&q=80",
  },
];
...
function Produk() {
  ...
  const [loadingIdx, setLoadingIdx] = useState(null); // Tambahkan state loading
            ...
            <AnimatePresence initial={false} custom={direction}>
              ...
              >
                {currentProduk.map((produk, idx) => (
                  <div
                    key={idx}
                    className="bg-gray-100 rounded-lg shadow hover:shadow-lg transition p-6 flex flex-col items-center"
                  >
                    <img
                      src={produk.gambar}
                      alt={produk.nama}
          ...
          {Array.from({ length: maxSlide }).map((_, idx) => (
            <span
              ...
              className={`w-3 h-3 rounded-full ${
                slide === idx ? "bg-blue-600" : "bg-gray-300"
              }`}
            />
          ))}
        </div>
